...
The `I2CResponder.poll_snd_rcv()` method then continually checks for Controller read and write requests, performing an `await uasyncio.sleep_ms(0)` only if there aren't any Controller requests. In other words, it should respond almost immediately when the Controller tries to send or receive data. This seemed to reduce the 
